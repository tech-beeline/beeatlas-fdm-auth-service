...
SELECT '/cx/api/cx/v1/cj/e2e', 'GET'
WHERE NOT EXISTS (
    SELECT 1 FROM user_auth.route
    WHERE path_mask = '/cx/api/cx/v1/cj/e2e' AND http_method = 'GET'
);
...
INSERT INTO user_auth.route (path_mask, http_method)
SELECT '/cx/api/cx/v1/bi-step/{code}', 'GET'
    WHERE NOT EXISTS (
    SELECT 1 FROM user_auth.route
    WHERE path_mask = '/cx/api/cx/v1/bi-step/{code}' AND http_method = 'GET'
);
